...
const Bookingscreen = ({match}) => {
    ...
    useEffect(() => {
       const user = JSON.parse(localStorage.getItem('currentuser'))
        ...
        const getroom = async () => {
            setLoading(true)
            const res = await axios.post('/api/getroombyid', {roomid:match.params.roomid})
            setRoom(res.data)
...
const onToken = async(token) => {
    ...
    const bookingDetails = {
        ...
        try {
            setLoading(true)
            const res = await axios.post('https://dry-ocean-12978.herokuapp.com/api/bookings/bookroom', bookingDetails)
            setLoading(false)
